...
bool AgendaService::deleteUser(const std::string userName,
    ...
    if (m_storage->deleteUser(t_filter_1) != 0) return true;
    else return false;
}
...
bool AgendaService::createMeeting(const std::string userName,
    ...
    if (m_storage->queryUser(t_filter_1).empty()) return false;

    auto t_filter_2 = [&](const Meeting &t_meeting) {
        return t_meeting.getTitle() == title;
    };
    // If there exist title conflict
    if (!m_storage->queryMeeting(t_filter_2).empty()) return false;
