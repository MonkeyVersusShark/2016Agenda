...
void AgendaUI::listAllUsers(void) {
  ...
  std::cout << std::setw(16) << "name" << std::setw(16) << "email"
            << "phone\n";
  std::list<User> t_list = m_agendaService.listAllUsers();
  for (auto object : t_list) {
    std::cout << std::setw(16) << object.getName() << std::setw(16)
              << object.getEmail() << std::setw(16) << object.getPhone()
              << std::endl;
  }
}
...
void AgendaUI::createMeeting(void) {
  ...
  if (m_agendaService.createMeeting(m_userName, t_title, t_startTime, t_endTime,
                                    t_participators))
