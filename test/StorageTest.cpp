...
TEST(StorageTest, UserCRUD) {
    ...
    auto test_switcher = [&](User &t_user) {
        t_user.setPhone("56565656");
    };
    ...
    EXPECT_EQ(3, test_instance->deleteUser([&](const User &t_user) { return t_user.getPhone() == "56565656"; }));

    std::list<User> test2;
    test2.push_back(test_user_1);
    test2.push_back(test_user_2);
    test2.push_back(test_user_3);
    std::list<User> result2 = test_instance->queryUser([&](const User &t_user) { return t_user.getEmail() == "GaauMatJe@citsin.com"; });
...
TEST(StorageTest, MeetingCRUD) {
    std::shared_ptr<Storage> test_instance = Storage::getInstance();
    std::vector<std::string> test_participators = { "Adam", "Philipe", "Emre", "Jordan", "Alberto" };

    Meeting test_meeting_1("Clown", test_participators, Date::stringToDate("1000-10-10/10:10"), Date::stringToDate("1000-12-12/12:12"), "t1");
    Meeting test_meeting_2("Clown", test_participators, Date::stringToDate("1001-10-10/10:10"), Date::stringToDate("1001-12-12/12:12"), "t2");
    Meeting test_meeting_3("Clown", test_participators, Date::stringToDate("1002-10-10/10:10"), Date::stringToDate("1002-12-12/12:12"), "t3");
    Meeting test_meeting_4("Clown", test_participators, Date::stringToDate("2000-10-10/10:10"), Date::stringToDate("2000-12-12/12:12"), "t4");
    Meeting test_meeting_5("Clown", test_participators, Date::stringToDate("2001-10-10/10:10"), Date::stringToDate("2001-12-12/12:12"), "t5");
    Meeting test_meeting_6("Clown", test_participators, Date::stringToDate("2002-10-10/10:10"), Date::stringToDate("2002-12-12/12:12"), "t6");
    ...
    auto test_switcher = [&](Meeting &t_meeting) {
        t_meeting.setTitle("t90");
    };
    ...
    EXPECT_EQ(3, test_instance->deleteMeeting([&](const Meeting &t_meeting) { return t_meeting.getTitle() == "t90"; }));

    std::list<Meeting> test2;
    test2.push_back(test_meeting_4);
    test2.push_back(test_meeting_5);
    test2.push_back(test_meeting_6);
    std::list<Meeting> result2 = test_instance->queryMeeting([&](const Meeting &t_meeting) { return t_meeting.getSponsor() == "Clown"; });
    EXPECT_EQ(test2, result2);
}
